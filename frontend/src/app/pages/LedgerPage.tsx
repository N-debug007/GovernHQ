...
export function LedgerPage() {
      ...
      apiGet('/monitoring/ledger?limit=50', token).then(res => {
        if (res.data?.rows) {
          const mapped = res.data.rows.map((row: any) => ({
            ...
            agent: map[row.agent_id] || row.agent_id?.slice(0, 8) + '...',
  ...
  return (
    <div className="bg-gradient-to-br from-[#0a0b14] via-[#111827] to-[#0f1629] border border-[#1e293b]/30 overflow-clip relative rounded-[40px] shadow-[0px_4px_90px_0px_rgba(0,0,0,0.5),0px_0px_100px_0px_rgba(59,130,246,0.1)] size-full backdrop-blur-xl" data-name="Ledger">
      ...
      <div className={`absolute left-[101px] top-0 bottom-0 overflow-y-auto transition-all ${showAnalytics ? 'right-[33.333%]' : 'right-0'}`}>
        <div className="p-[56px] pl-[39px]">
          <div className="flex items-center justify-between mb-[12px]">
            <h1 className="font-['Mulish:Bold',sans-serif] font-bold text-white text-[28px] tracking-tight">Ledger</h1>
            <button
          ...
          <div className="bg-gradient-to-br from-[#1e293b]/50 to-[#0f172a]/30 border border-[#334155]/30 rounded-[16px] shadow-[0px_8px_32px_0px_rgba(0,0,0,0.3)] backdrop-blur-xl p-[32px]">
                ...
                <tbody>
                  {filteredEntries.map((entry, index) => (
                    <>
                      ...
                      {selectedEntry === index && (
                        <tr key={`detail-${index}`}>
                          <td colSpan={5} className="pb-[16px]">
            ...
            <div className="flex items-center justify-between mt-[32px] pt-[24px] border-t border-[#334155]/30">
              ...
              <div className="flex items-center gap-[8px]">
                <span className={chainStatus?.valid ? "text-[#10b981]" : chainStatus === null ? "text-[#64748b]" : "text-[#f59e0b]"}>⚡</span>
                <p className={`text-[14px] font-semibold ${chainStatus?.valid ? "text-[#10b981]" : chainStatus === null ? "text-[#64748b]" : "text-[#f59e0b]"}`}>
                  {chainStatus === null
                    ? "Checking chain..."
                    : chainStatus.valid
                    ? `Chain Valid (${chainStatus.total} rows)`
                    : `Chain Integrity (${chainStatus.total} rows)`}
                </p>
              </div>
            </div>
          </div>
      ...
      {showAnalytics && (
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-br from-[#1e293b] to-[#0f172a] border-l border-[#1e293b]/50 backdrop-blur-xl p-[48px] overflow-y-auto z-10">
          <div className="flex items-center justify-between mb-[32px]">
            <h2 className="font-['Mulish:SemiBold',sans-serif] font-semibold text-white text-[22px]">Analytics</h2>
            <button
              ...
              <svg className="size-[16px]" fill="none" viewBox="0 0 16 16">
                <path d="M12 4L4 12M4 4L12 12" stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:stroke-[#ef4444]" />
              </svg>
            </button>
          </div>
          ...
          <div className="space-y-[16px]">
            ...
            <div className={`bg-gradient-to-br from-[#1e293b]/50 to-[#0f172a]/30 border rounded-[16px] p-[20px] ${
              ...
              <p className={`text-[20px] font-semibold ${
                ...
                'text-[#f59e0b]'}`}>
                {chainStatus === null ? 'Checking...' : chainStatus.valid ? '✓ Valid' : '⚠ Partial'}
              </p>
              {chainStatus && (
